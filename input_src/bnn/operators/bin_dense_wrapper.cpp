...
void bin_dense(
	...
	//printf("layer_type = %d, d_i_idx = %d, d_o_idx = %d, o_index = %d, n_inputs = %d, n_outputs = %d\n",
	//(int) layer_type, (int) d_i_idx, (int) d_o_idx, (int) o_index, (int) n_inputs, (int) n_outputs);
	...
	for (Address o = 0; o < n_outputs; o++) {
		...
		unsigned int o_addr = (o_index+o)/WORD_SIZE;
		...
		LOOP_DENSE_I:
		for (Address i = 0; i < n_inputs; i=i+CONVOLVERS*WORD_SIZE) {
			#pragma HLS PIPELINE
			#pragma HLS LOOP_TRIPCOUNT min=1 max=64
			unsigned int wt_addr = (o*n_inputs+i) / WORD_SIZE;
			for (IdxType j = 0; j < CONVOLVERS; j++) {
				...
				//if(local_cnt<10){ printf("x=%08x%08x=>", (unsigned int) x(63, 32), (unsigned int) x(31, 0)); }
				x = x - ((x >> 1) & m1);
				x = (x & m2) + ((x >> 2) & m2);
				x = (x + (x >> 4)) & m4;
				x = x + (x >> 8);
				...
				x = x + (x >> 32);
				x = x & ((Word) 0x7f);
				//if(local_cnt<10){ printf("%d\n", (unsigned int) x); }
				sum_m[j] = WORD_SIZE - (DenseSum)(x<<1);
			}

			for (IdxType j = 0; j < CONVOLVERS; j++){ sum = sum + sum_m[j]; }
		} // n_inputs
...
#pragma HLS PIPELINE
				Output_1.write(2048);
				Output_1.write(2048);
				bit32 out_tmp_h, out_tmp_l;
				out_tmp_h(31, 0) = dmem[0][j][k](63, 32);
				Output_1.write(out_tmp_h);
				out_tmp_l(31, 0) = dmem[0][j][k](31,  0);
				Output_1.write(out_tmp_l);
