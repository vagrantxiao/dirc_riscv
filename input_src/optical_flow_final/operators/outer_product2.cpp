...
#pragma HLS interface ap_hs port=Output_1
  OUTER_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    ...
    OUTER_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      databus_t out_tmp;
      out_tmp = out.val[3].range(31,0);
      Output_1.write(out_tmp);
      //printf("out1: %08x\n", out_tmp.to_int());

      out_tmp(15,0) = out.val[3].range(47,32);
      out_tmp(31,16) = out.val[4].range(15,0);
      Output_1.write(out_tmp);
      //printf("out1: %08x\n", out_tmp.to_int());

      out_tmp(31,0) = out.val[4].range(47,16);
      Output_1.write(out_tmp);
      //printf("out1: %08x\n", out_tmp.to_int());

      out_tmp(31,0) = out.val[5].range(31,0);
      Output_1.write(out_tmp);
      //printf("out1: %08x\n", out_tmp.to_int());

      out_tmp(15,0) = out.val[5].range(47,32);
      out_tmp(31,16) = 0;
      Output_1.write(out_tmp);
      //printf("out1: %08x\n", out_tmp.to_int());
