  ...
  TENSOR_WEIGHT_Y_OUTER: for(int r=0; r<MAX_HEIGHT+1; r++)
  {
    ...
    TENSOR_WEIGHT_Y_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      if(r >= 1)
      {
    	...
    	widetemp(15, 0) = acc.val[0].range(47,32);
    	widetemp(31,16) = acc.val[1].range(15, 0);
    	Output_1.write(widetemp);

    	widetemp(31, 0) = acc.val[1].range(47, 16);
    	Output_1.write(widetemp);

    	widetemp(31, 0) = acc.val[2].range(31, 0);
    	Output_1.write(widetemp);

    	widetemp(15, 0) = acc.val[2].range(47, 32);
    	widetemp(31, 16) = 0;
    	Output_1.write(widetemp);
